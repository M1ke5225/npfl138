...
class Model(npfl138.TrainableModule):
    def __init__(self, args: argparse.Namespace, train: MorphoDataset.Dataset) -> None:
        ...
        # and we will sum the outputs of the forward and backward directions.
        self._word_rnn = ...
    ...
    def constrained_decoding(self, logits: torch.Tensor, word_ids: torch.Tensor) -> torch.Tensor:
        ...
        # the `I-TYPE` tag must follow either the `B-TYPE` or the `I-TYPE` tag. This
        # correctness can be implemented by checking that every neighboring pair of tags
        # is valid according to the transition matrix `self._A`, plus the sequence cannot
        # start with an "I-" tag -- a possible solution is to consider a tag sequence to
        # be prefixed by a virtual "O" tag during decoding. Finally, the tags for padding
        # tokens must be `MorphoDataset.PAD`.
...
def main(args: argparse.Namespace) -> dict[str, float]:
    ...
    model.configure(
        ...
        metrics={
            ...
            # TODO: Create `npfl138.metrics.BIOEncodingF1Score` for constrained decoding and also
            # for greedy decoding, passing both `list(morpho.train.tags.string_vocab)`
